...
## Split relay для России + Финляндии

Если нужен режим, где пользователь подключается к российскому VPS, российские сайты выходят напрямую с российского IP, а иностранный трафик уходит через финский exit, используйте split relay:

```bash
meridian relay split-deploy RU_RELAY_IP --exit FI_EXIT_IP --yes
```

Если команда запускается прямо на российском VPS, `RU_RELAY_IP` можно не указывать:

```bash
meridian relay split-deploy --exit FI_EXIT_IP --yes
```

Что делает команда:

1. Разворачивает на российском VPS полноценный Xray/XHTTP вход без Telegram-бота
2. Копирует существующих клиентов с финского exit на российский split relay
3. Настраивает Xray routing на российском сервере:
   - `geosite:category-ru` и `geoip:ru` → `direct`
   - всё остальное → финский exit через VLESS+XHTTP outbound
4. Записывает split relay в credentials финского сервера
5. Обновляет Telegram-бота на финском сервере, чтобы новые подписки синхронизировались на российский split relay

В HAPP пользователь получает один профиль. Название профиля остаётся брендингом финского сервера, но адрес подключения указывает на российский split relay.
