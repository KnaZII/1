...
class TestWarpPipelineIntegration:
    ...
    def test_discord_bypass_after_warp_before_verify(self):
        """Discord direct routing must override WARP before verification."""
        from meridian.provision import build_setup_steps

        ctx = ProvisionContext(ip="198.51.100.1", creds_dir="/tmp/test", warp=True)
        steps = build_setup_steps(ctx)
        step_names = [s.name for s in steps]
        warp_idx = step_names.index("Configure WARP outbound")
        discord_idx = step_names.index("Configure Discord RTC bypass")
        verify_idx = step_names.index("Verify Xray configuration")
        assert warp_idx < discord_idx < verify_idx

    def test_private_network_blocking_before_verify(self):
        from meridian.provision import build_setup_steps

        ctx = ProvisionContext(ip="198.51.100.1", creds_dir="/tmp/test", warp=False)
        steps = build_setup_steps(ctx)
        step_names = [s.name for s in steps]
        assert "Configure private-network blocking" in step_names
        assert step_names.index("Configure private-network blocking") < step_names.index("Verify Xray configuration")
