...
# ---------------------------------------------------------------------------
# ConfigureDiscordRtcBypass
# ---------------------------------------------------------------------------


class TestConfigureDiscordRtcBypass:
    """Tests for Discord RTC direct routing."""

    def _make_xray_template(self, outbounds=None, routing=None):
        template = {"log": {}, "outbounds": outbounds or [{"protocol": "socks", "tag": "warp"}]}
        if routing is not None:
            template["routing"] = routing
        return template
    ...
    def _panel_returning(self, template):
        ...
        panel = MagicMock()
        wrapper = json.dumps({"xraySetting": json.dumps(template)})
        panel.api_post_empty.return_value = {"success": True, "obj": wrapper}
        panel.api_post_form.return_value = {"success": True}
        return panel
    ...
    def _run_step(self, panel):
        from meridian.provision.steps import ProvisionContext
        from meridian.provision.xray import ConfigureDiscordRtcBypass
        from tests.provision.conftest import MockConnection

        step = ConfigureDiscordRtcBypass()
        ctx = ProvisionContext(ip="198.51.100.1", creds_dir="/tmp")
        ctx["panel"] = panel
        return step.run(MockConnection(), ctx)

    def test_adds_direct_outbound_and_discord_rule(self):
        template = self._make_xray_template()
        panel = self._panel_returning(template)
        ...
        form_data = panel.api_post_form.call_args[0][1]
        saved = json.loads(unquote(form_data.split("xraySetting=")[1]))

        assert any(o == _DIRECT_OUTBOUND for o in saved["outbounds"])
        assert saved["routing"]["rules"][:3] == [_DISCORD_RTC_RULE] + _DISCORD_RTC_UDP_RULES

    def test_idempotent_when_already_configured(self):
        template = self._make_xray_template(
            outbounds=[{"protocol": "socks", "tag": "warp"}, _DIRECT_OUTBOUND],
            routing={"rules": [_DISCORD_RTC_RULE] + _DISCORD_RTC_UDP_RULES},
        )
        panel = self._panel_returning(template)
...
# ---------------------------------------------------------------------------
# ConfigurePrivateNetworkBlocking
# ---------------------------------------------------------------------------


class TestConfigurePrivateNetworkBlocking:
    """Tests for private-network destination blocking."""
    ...
    def _run_step(self, panel):
        from meridian.provision.steps import ProvisionContext
        from meridian.provision.xray import ConfigurePrivateNetworkBlocking
        from tests.provision.conftest import MockConnection

        step = ConfigurePrivateNetworkBlocking()
        ctx = ProvisionContext(ip="198.51.100.1", creds_dir="/tmp")
        ctx["panel"] = panel
        return step.run(MockConnection(), ctx)

    def test_adds_blocked_outbound_and_private_rule(self):
        template = self._make_xray_template()
        panel = self._panel_returning(template)
        ...
        assert any(o == _BLOCKED_OUTBOUND for o in saved["outbounds"])
        assert saved["routing"]["rules"][0] == _PRIVATE_NETWORK_RULE

    def test_idempotent_when_already_configured(self):
        template = self._make_xray_template(
            outbounds=[{"protocol": "freedom", "tag": "direct"}, _BLOCKED_OUTBOUND],
            routing={"rules": [_PRIVATE_NETWORK_RULE]},
        )
        panel = self._panel_returning(template)
