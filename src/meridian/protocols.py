...
class Protocol(ABC):
    ...
    def _build_fragment(self, name: str, server_name: str = "", extra_suffix: str = "") -> str:
        ...
        base = server_name or name
        return f"#{quote(base + extra_suffix + self.url_suffix)}"
...
class XHTTPProtocol(Protocol):
    ...
    def build_relay_url(
        ...
        *,
        ...
        route_path = xhttp_path or creds.xhttp.xhttp_path or ""
        if not route_path:
            return ""
        uuid = self._resolve_uuid(reality_uuid, wss_uuid)
        via = f"-via-{relay_name}" if relay_name else f"-via-{relay_ip}"
        host = public_host or relay_ip
        return self.build_url(
            uuid,
            name,
            ip=creds.server.ip or "",
            port=relay_port,
            xhttp_path=route_path,
            domain=creds.server.domain or "",
            sni=relay_sni,
            connect_host=_bracket_ipv6(host),
            server_name=server_name,
            extra_suffix=via,
        )
