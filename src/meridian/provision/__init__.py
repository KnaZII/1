...
def build_setup_steps(ctx: ProvisionContext) -> list[Step]:
    ...
    steps.append(ConfigureDiscordRtcBypass())
    steps.append(ConfigurePrivateNetworkBlocking())
    steps.append(ConfigureTorrentBlocking())
    ...
    if ctx.needs_web_server:
        ...
        if ctx.telegram_bot:
            steps.append(DeployTelegramBot())
