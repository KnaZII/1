...
_XRAY_TORRENT_DETECTION_LOG_CONFIG = {
...
class ConfigureTorrentDetectionLogs:
    """Enable access logs needed to map BitTorrent attempts to client emails."""

    name = "Configure torrent detection logs"
    ...
    def run(self, conn: ServerConnection, ctx: ProvisionContext) -> StepResult:
        ...
        conn.run("mkdir -p /var/log/xray && chmod 755 /var/log/xray", timeout=15)

        template, error = _load_xray_template(panel)
        if error:
            return StepResult(name=self.name, status="failed", detail=error)

        current_log = template.get("log", {})
        if current_log == _XRAY_TORRENT_DETECTION_LOG_CONFIG:
            return StepResult(name=self.name, status="ok", detail="Torrent detection logs already configured")

        template["log"] = dict(_XRAY_TORRENT_DETECTION_LOG_CONFIG)
        error = _save_xray_template(panel, template)
        if error:
            return StepResult(name=self.name, status="failed", detail=error)

        return StepResult(name=self.name, status="changed", detail="Enabled Xray access log for torrent detection")
...
_DISCORD_RTC_RULE = {
    "type": "field",
    "outboundTag": "direct",
    "domain": _DISCORD_RTC_DOMAINS,
}

_DISCORD_RTC_UDP_RULES = [
    {
        "type": "field",
        "outboundTag": "direct",
        "network": "udp",
...
def _vless_xhttp_outbound(
    *,
    ...
    """Build a VLESS+TLS+XHTTP outbound used by split relay chaining."""
    return {
        "protocol": "vless",
        "tag": tag,
        "settings": {
...
def _load_xray_template(panel: PanelClient) -> tuple[dict[str, Any], str | None]:
    """Fetch and parse the 3x-ui Xray template.

    Returns ``(template, error)``. The helper keeps routing patch steps small
    and consistent without changing their public behavior.
    """
    try:
        data = panel.api_post_empty("/panel/xray/")
    except PanelError as e:
        return {}, f"Failed to fetch Xray config: {e}"

    if not data.get("success"):
        return {}, "Failed to fetch Xray config template"
    ...
    try:
        ...
        return {}, f"Failed to parse Xray template: {e}"

    return template, None


def _save_xray_template(panel: PanelClient, template: dict[str, Any]) -> str | None:
    """Save an updated Xray template. Returns an error string on failure."""
    updated_json = json.dumps(template)
    try:
        from urllib.parse import quote as urlquote
        ...
        return f"Failed to save Xray config: {e}"

    if not save_data.get("success"):
        return f"Save failed: {save_data.get('msg', 'unknown')}"
...
def _ensure_outbound(template: dict[str, Any], outbound: dict[str, Any]) -> bool:
    """Ensure an outbound exists by tag. Returns True when changed."""
...
def _upsert_outbound(template: dict[str, Any], outbound: dict[str, Any]) -> bool:
    """Create or replace an outbound by tag. Returns True when changed."""
    outbounds = template.get("outbounds", [])
    tag = outbound.get("tag")
...
def _ensure_rule_first(template: dict[str, Any], rule: dict[str, Any]) -> bool:
    """Ensure a routing rule exists and is ordered before generic rules."""
    routing = template.setdefault("routing", {})
    rules = routing.setdefault("rules", [])
    if any(r == rule for r in rules):
        return False
    rules.insert(0, dict(rule))
    return True


class ConfigureDiscordRtcBypass:
    """Route Discord RTC traffic directly, bypassing WARP/default proxies.

    Discord voice/video is sensitive to UDP/WebRTC behavior. When WARP is the
    default outbound, Discord RTC may fail to establish. This rule keeps normal
    VPN behavior for everything else while sending Discord domains through the
    regular server egress path.
    """

    name = "Configure Discord RTC bypass"
    ...
    def run(self, conn: ServerConnection, ctx: ProvisionContext) -> StepResult:
        ...
        changed = _ensure_outbound(template, _DIRECT_OUTBOUND)
        for rule in reversed(_DISCORD_RTC_UDP_RULES):
            changed = _ensure_rule_first(template, rule) or changed
        changed = _ensure_rule_first(template, _DISCORD_RTC_RULE) or changed

        if not changed:
            return StepResult(name=self.name, status="ok", detail="Discord RTC bypass already configured")

        error = _save_xray_template(panel, template)
        if error:
            return StepResult(name=self.name, status="failed", detail=error)

        return StepResult(name=self.name, status="changed", detail="Discord domains and RTC UDP route via direct outbound")


class ConfigurePrivateNetworkBlocking:
    """Block proxied clients from reaching private/local server networks."""

    name = "Configure private-network blocking"
    ...
    def run(self, conn: ServerConnection, ctx: ProvisionContext) -> StepResult:
        ...
        changed = _ensure_outbound(template, _BLOCKED_OUTBOUND)
        changed = _ensure_rule_first(template, _PRIVATE_NETWORK_RULE) or changed

        if not changed:
            return StepResult(name=self.name, status="ok", detail="Private-network blocking already configured")

        error = _save_xray_template(panel, template)
        if error:
            return StepResult(name=self.name, status="failed", detail=error)

        return StepResult(name=self.name, status="changed", detail="Blocked geoip:private destinations")


class ConfigureTorrentBlocking:
    """Block BitTorrent protocol traffic at the Xray routing level."""

    name = "Configure torrent blocking"
    ...
    def run(self, conn: ServerConnection, ctx: ProvisionContext) -> StepResult:
        ...
        changed = _ensure_outbound(template, _BLOCKED_OUTBOUND)
        changed = _ensure_rule_first(template, _BITTORRENT_RULE) or changed

        if not changed:
            return StepResult(name=self.name, status="ok", detail="Torrent blocking already configured")

        error = _save_xray_template(panel, template)
        if error:
            return StepResult(name=self.name, status="failed", detail=error)

        return StepResult(name=self.name, status="changed", detail="Blocked BitTorrent protocol traffic")
...
class ConfigureSplitRelayRouting:
    """Route Russian destinations directly and all other traffic via an exit."""

    name = "Configure split relay routing"
    ...
    def __init__(
        ...
        *,
        ...
        self.exit_sni = exit_sni
        self.exit_xhttp_path = exit_xhttp_path

    def run(self, conn: ServerConnection, ctx: ProvisionContext) -> StepResult:
        panel = ctx.panel
        if panel is None:
            return StepResult(name=self.name, status="failed", detail="No panel client in context")
        if not self.exit_host or not self.exit_uuid or not self.exit_xhttp_path:
            return StepResult(name=self.name, status="failed", detail="Missing exit upstream credentials")

        template, error = _load_xray_template(panel)
        if error:
            return StepResult(name=self.name, status="failed", detail=error)

        changed = _upsert_outbound(
            template,
            _vless_xhttp_outbound(
                tag="fi-exit",
                address=self.exit_host,
                port=self.exit_port,
                uuid=self.exit_uuid,
                server_name=self.exit_sni or self.exit_host,
                xhttp_path=self.exit_xhttp_path,
            ),
        )
        changed = _ensure_outbound(template, _DIRECT_OUTBOUND) or changed
        ...
        rules = [
            ...
            for r in rules
            if not (
                r.get("outboundTag") in {"direct", "fi-exit"}
                and (
                    "geosite:category-ru" in r.get("domain", [])
                    or "geoip:ru" in r.get("ip", [])
                    or r.get("network") == "tcp,udp"
                )
            )
        ]
        desired_rules = [dict(r) for r in _RU_DIRECT_RULES]
        desired_rules.append({"type": "field", "network": "tcp,udp", "outboundTag": "fi-exit"})
        if rules[: len(desired_rules)] != desired_rules:
            routing["rules"] = desired_rules + rules
            changed = True
        else:
            routing["rules"] = rules

        if not changed:
            return StepResult(name=self.name, status="ok", detail="Split relay routing already configured")
        ...
        return StepResult(
            name=self.name,
            status="changed",
            detail=f"RU direct, default via {self.exit_host}:{self.exit_port}",
        )
