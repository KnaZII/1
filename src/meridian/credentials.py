...
class RelayEntry:
    ...
    mode: str = "tcp"  # "tcp" = dumb Realm forwarder, "split" = Xray policy relay
    panel_url: str = ""  # split relay panel URL, used by the Telegram bot for client sync
    panel_username: str = ""
    panel_password: str = ""
    xhttp_path: str = ""  # split relay public XHTTP path
    public_host: str = ""  # host clients connect to; defaults to ip
    _extra: dict[str, Any] = field(default_factory=dict, repr=False)
...
class BrandingConfig:
    ...
    profile_name: str = ""  # connection profile/server name shown in client apps
...
class ServerCredentials:
    ...
    def save(self, path: Path) -> None:
        ...
        for field_name in ("server_name", "profile_name", "icon", "color"):
...
def _load_v2(data: dict[str, Any]) -> ServerCredentials:
    ...
    branding = _load_known_dataclass(
        branding_data,
        BrandingConfig,
        {"server_name", "profile_name", "icon", "color"},
        defaults={"server_name": "", "profile_name": "", "icon": "", "color": ""},
    )
