...
def _regenerate_client_pages(
    ...
    from meridian.config import DEFAULT_PROFILE_NAME, DEFAULT_SERVER_ICON
    from meridian.render import save_connection_html
    from meridian.urls import (
        build_all_relay_urls,
        build_protocol_urls,
        generate_qr_base64,
        public_protocol_urls,
        public_relay_url_sets,
    )
    ...
    for client in creds.clients:
        ...
        public_urls = public_protocol_urls(protocol_urls)
        relay_url_sets = public_relay_url_sets(build_all_relay_urls(
            client.name,
            client.reality_uuid,
            client.wss_uuid,
            creds,
        ...
        if creds.server.hosted_page and creds.panel.info_page_path and client.reality_uuid:
            ...
            urls_with_qr = [dc_replace(p, qr_b64=generate_qr_base64(p.url)) if p.url else p for p in public_urls]
...
def _xhttp_inbound_or_fail(panel: object) -> object:
    """Return the XHTTP inbound from a PanelClient-like object."""
    from meridian.panel import PanelError
    from meridian.protocols import get_protocol

    xhttp_proto = get_protocol("xhttp")
    if xhttp_proto is None:
        raise PanelError("XHTTP protocol is not registered")
    inbounds = panel.list_inbounds()
    ib = xhttp_proto.find_inbound(inbounds)
    if ib is None:
        raise PanelError("VLESS-Reality-XHTTP inbound not found")
    return ib


def _sync_clients_to_split_relay(
    relay_conn: ServerConnection,
    relay_creds: ServerCredentials,
    exit_creds: ServerCredentials,
) -> None:
    """Copy existing exit clients into a split relay XHTTP inbound."""
    from meridian.panel import PanelClient, PanelError

    panel = PanelClient(
        conn=relay_conn,
        panel_port=relay_creds.panel.port,
        web_base_path=relay_creds.panel.web_base_path or "",
    )
    panel.login(relay_creds.panel.username or "", relay_creds.panel.password or "")
    with panel:
        ib = _xhttp_inbound_or_fail(panel)
        existing = {c.get("id", "") for c in ib.clients}
        for client in exit_creds.clients:
            if not client.reality_uuid or client.reality_uuid in existing:
                continue
            settings = {
                "clients": [
                    {
                        "id": client.reality_uuid,
                        "flow": "",
                        "email": f"xhttp-{client.name}",
                        "limitIp": 0,
                        "totalGB": 0,
                        "expiryTime": 0,
                        "enable": True,
            ...
            try:
                panel.add_client(ib.id, settings)
            except PanelError as exc:
                warn(f"Could not sync client {client.name} to split relay: {exc}")


def _configure_split_relay_routing(
    relay_conn: ServerConnection,
    relay_creds: ServerCredentials,
    exit_creds: ServerCredentials,
) -> None:
    """Patch the split relay Xray template with RU-direct/FI-default routing."""
    from meridian.panel import PanelClient
    from meridian.provision.steps import ProvisionContext
    from meridian.provision.xray import ConfigureSplitRelayRouting
    ...
    ctx = ProvisionContext(
        ip=relay_creds.server.ip or "",
        user="root",
        sni=relay_creds.server.sni or DEFAULT_SNI,
        creds_dir="",
    )
    ctx.credentials = relay_creds
    ctx.panel = panel

    exit_host = exit_creds.server.domain or exit_creds.server.ip or ""
    step = ConfigureSplitRelayRouting(
        exit_host=exit_host,
        exit_port=443,
        exit_uuid=exit_creds.reality.uuid or "",
        exit_sni=exit_host,
        exit_xhttp_path=exit_creds.xhttp.xhttp_path or "",
    )
    result = step.run(relay_conn, ctx)
    panel.cleanup()
    if result.status == "failed":
        fail("Split relay routing failed", hint=result.detail, hint_type="system")


def _redeploy_exit_bot(resolved_exit: ResolvedServer, exit_creds: ServerCredentials) -> None:
    """Update the Telegram bot script on the exit so it knows split relays."""
    from meridian.provision.services import DeployTelegramBot
    from meridian.provision.steps import ProvisionContext

    ctx = ProvisionContext(
        ip=exit_creds.server.ip or resolved_exit.ip,
        user=resolved_exit.user,
        sni=exit_creds.server.sni or DEFAULT_SNI,
        creds_dir=str(resolved_exit.creds_dir),
    )
    ctx.credentials = exit_creds
    result = DeployTelegramBot().run(resolved_exit.conn, ctx)
    if result.status == "failed":
        warn(f"Could not update Telegram bot on exit: {result.detail}")


def run_split_deploy(
    relay_ip: str,
    exit_arg: str,
    user: str = "root",
    relay_name: str = "",
    ...
    """Deploy a split relay: RU destinations direct, everything else via exit."""
    if not is_ip(relay_ip) and not is_local_keyword(relay_ip):
        fail(f"Invalid relay IP: {relay_ip}", hint="Enter a valid IP address or 'local'", hint_type="user")
    ...
    if not exit_creds.xhttp.xhttp_path:
        fail(
            "Exit server has no XHTTP path",
            hint=f"Redeploy the exit first: meridian deploy {resolved_exit.ip} --yes",
            hint_type="system",
        )
    if not exit_creds.clients:
        fail(
            "Exit server has no clients to use as upstream credentials",
            hint=f"Create a client first: meridian client add default --server {resolved_exit.ip}",
            hint_type="user",
        )
    ...
    if not yes:
        ...
        summary = (
            f"Split relay: {user}@{relay_ip}:443\n"
            f"Exit:        {resolved_exit.ip}:443\n"
            f"RU traffic:  direct from {relay_ip}\n"
            f"Other:       via {resolved_exit.ip}\n"
            f"Bot:         remains on exit only\n"
            f"Subscription URLs will point clients to the split relay."
        )
        err_console.print()
        err_console.print(Panel(summary, title="[bold]Split relay deployment plan[/bold]", border_style="cyan"))
        err_console.print()
        confirm(f"Deploy split relay to {user}@{relay_ip}?")

    first_client = exit_creds.clients[0].name or "default"
    from meridian.commands import setup as setup_cmd
    ...
    setup_cmd.run(
        ...
        yes=True,
        harden=True,
        server_name=exit_creds.branding.server_name,
        icon=exit_creds.branding.icon,
        color=exit_creds.branding.color,
        pq=False,
        warp=False,
        geo_block=False,
        ssh_port=ssh_port,
    ...
    relay_creds_dir = SERVER_CREDS_DIR if is_local_keyword(relay_ip) else CREDS_BASE / sanitize_ip_for_path(relay_ip)
    relay_creds = ServerCredentials.load(relay_creds_dir / "proxy.yml")
    relay_public_ip = relay_creds.server.ip or relay_ip
    relay_conn = ServerConnection(
        ip=relay_public_ip,
        user=user,
        local_mode=is_local_keyword(relay_ip),
        port=ssh_port,
    )
    if not relay_conn.local_mode:
        relay_conn.check_ssh()

    info("Syncing existing clients to split relay...")
    _sync_clients_to_split_relay(relay_conn, relay_creds, exit_creds)
    ok("Split relay clients synced")

    info("Configuring split routing on relay...")
    _configure_split_relay_routing(relay_conn, relay_creds, exit_creds)
    ok("Split routing configured")

    relay_entry = RelayEntry(
        ip=relay_public_ip,
        name=relay_name or "ru-split",
        port=443,
        added=datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        sni=relay_creds.server.domain or relay_creds.server.ip or relay_public_ip,
        mode="split",
        panel_url=relay_creds.panel.url,
        panel_username=relay_creds.panel.username or "",
        panel_password=relay_creds.panel.password or "",
        xhttp_path=relay_creds.xhttp.xhttp_path or "",
        public_host=relay_creds.server.domain or relay_creds.server.ip or relay_public_ip,
    )
    exit_creds.relays = [r for r in exit_creds.relays if not (r.ip == relay_public_ip and r.mode == "split")]
    exit_creds.relays.append(relay_entry)
    _save_exit_credentials_with_sync(
        resolved_exit,
        exit_creds,
        recovery_hint=f"Retry after SSH/SCP works: meridian relay split-deploy {relay_public_ip} --exit {resolved_exit.ip}",
    )
    registry.add(ServerEntry(host=relay_public_ip, user=user, name=relay_entry.name, role=SERVER_ROLE_RELAY, port=ssh_port))

    if exit_creds.clients:
        info("Regenerating exit-hosted client pages...")
        _regenerate_client_pages(resolved_exit, exit_creds)
    _redeploy_exit_bot(resolved_exit, exit_creds)

    err_console.print()
    ok(f"Split relay {relay_public_ip} is attached to exit {resolved_exit.ip}")
    err_console.print("  [dim]Clients connect to RU split relay; RU destinations go direct, other destinations go through FI exit.[/dim]")
    err_console.print("  [dim]Telegram bot remains on the exit server and will sync new subscriptions to the split relay.[/dim]\n")
