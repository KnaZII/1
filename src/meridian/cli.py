...
@relay_app.command("split-deploy")
def relay_split_deploy_cmd(
    relay_ip: str = typer.Argument("local", help="Russian split relay server IP address, or 'local'"),
    exit: str = typer.Option(..., "--exit", "-e", help="Finland exit server (IP or name)"),
    user: str = typer.Option("root", "--user", "-u", help="SSH user"),
    name: str = typer.Option("ru-split", "--name", help="Friendly name for the split relay"),
    sni: str = typer.Option("", "--sni", help="Relay Reality SNI target"),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation"),
    ssh_port: int = typer.Option(22, "--ssh-port", help="SSH port on the relay server"),
) -> None:
    """Deploy a split relay: Russian traffic direct, everything else via exit.

    [dim]Example:[/dim]
      [cyan]meridian relay split-deploy --exit 5.6.7.8 --yes[/cyan]
      [cyan]meridian relay split-deploy 1.2.3.4 --exit 5.6.7.8 --yes[/cyan]
    """
    from meridian.commands.relay import run_split_deploy

    run_split_deploy(relay_ip, exit, user, name, yes=yes, sni=sni, ssh_port=ssh_port)
